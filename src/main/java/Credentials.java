...
//http://api.wunderground.com/api/125556e731c003cb/conditions/q/CA/San_Francisco.xml REMOVE BEFORE UPLOADING
    private static boolean populate() {
        ...
        if (url.length() < 2){
            ...
            port=Integer.parseInt(doc.getElementsByAttribute("port").attr("port"));

            //Get the table name, this time demonstrating one line instead of 3
            table = doc.select("table").first().text();
            user=doc.getElementsByAttribute("userId").attr("UserId");
            password=doc.getElementsByAttribute("password").attr("password");
            apiKey=doc.getElementsByAttribute("key").attr("key");
